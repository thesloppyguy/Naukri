...
const createGeneralQuery = async (params, page) => {
    ...
    if (params.department in department_1.default) {
        for (let i = 0; i < department_1.default[params.department].length; i++) {
            should.push({
                match_phrase: { review_body: department_1.default[params.department][i] },
            });
        }
    }
    if (params.industry in industry_1.default) {
        for (let i = 0; i < industry_1.default[params.industry].length; i++) {
            should.push({
                match_phrase: { review_body: industry_1.default[params.industry][i] },
            });
        }
    }
    if (params.location !== '') {
    ...
    if (params.jobcode !== '') {
        ...
        if (description === '') {
            throw new graphql_1.GraphQLError('Jobcode not found', {
