...
const cleanText = (text: string) => {
  ...
  const finalWordList: string[] = Array.from(uniqueWords).filter(
    (word) => word.length > 1,
  );
  const removedWordList: string[] = Array.from(finalWordList).filter(
...
const createGeneralQuery = async (params: SearchBody, page: number) => {
  ...
  if (params.department in department) {
    for (let i = 0; i < department[params.department].length; i++) {
      should.push({
        match_phrase: { review_body: department[params.department][i] },
      });
    }
  }
  if (params.industry in industry) {
    for (let i = 0; i < industry[params.industry].length; i++) {
      should.push({
        match_phrase: { review_body: industry[params.industry][i] },
      });
    }
  }
  if (params.location !== '') {
  ...
  if (params.currentDesignation !== '') {
    must.push({
      more_like_this: {
        fields: ['review_body'],
        like: params.currentDesignation,
        min_term_freq: 1,
        min_doc_freq: 1,
      },
    });
  ...
  if (params.jobcode !== '') {
    ...
    if (description === '') {
      throw new GraphQLError('Jobcode not found', {
