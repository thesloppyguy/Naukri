...
const resolvers: Resolvers = {
  Query: {
    ...
    getUserByOrganization: async (_, __, context) => {
      const userList = await UserModel.find({ organization: context.user.organization._id }).exec()
  ...
  Mutation: {
    registerOrganization: async (_, { input }) => {
      ...
      const url = input.url
      const existingOrganizations = await OrgnisationModel.findOne({ name: name, contactEmail: contactEmail }).exec()
      if (existingOrganizations !== null) {
        throw new GraphQLError("Organization already taken. Please check your email for an Invite.", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
    ...
    inviteOrganization: async (_, { input }) => {
      ...
      if (name == '' || contactEmail == '') {
        throw new GraphQLError("Paramenter Missing", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
      const existingOrganizations = await OrgnisationModel.findOne({ name })
      if (existingOrganizations !== null) {
        throw new GraphQLError("Organization already taken.", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
      await OrgnisationModel.create({
        name,
        contactEmail,
    ...
    deleteOrganization: async (_, { id }) => {
      ...
      if (!org) {
        throw new GraphQLError("User not found", { extensions: { code: 'CUSTOM_CODE_400' }, })
      }
    ...
    updateOrganizationStatus: async (_, { input }) => {
      ...
      if (organization == null) {
        throw new GraphQLError("Organization Not Found", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
    ...
    inviteUser: async (_, { input }, context) => {
      ...
      if (name == '' || email == '') {
        throw new GraphQLError("Parameter Missing", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
    ...
    loginUser: async (_, { input: { organization, email, password } }) => {
      ...
      if (org == null) {
        throw new GraphQLError("Invalid Organization", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
      const user = await UserModel.findOne({
        $and: [{ email: email }, { organization: org._id }],
      }).populate('organization').exec();
      if (user == null) {
        throw new GraphQLError("Invalid Credentails", { extensions: { code: 'CUSTOM_CODE_400' }, });
      }
      ...
      throw new GraphQLError("Invalid Credintials", { extensions: { code: 'CUSTOM_CODE_400' }, });
    },
  }
}
